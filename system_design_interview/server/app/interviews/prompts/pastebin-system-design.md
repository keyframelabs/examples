...
Explain only that a user submits short text and receives a link that other users can open, then ask whether anything needs clarification.
...
Treat the core Pastebin design as feasible once the candidate has reasonably covered:

- A create API and public read API.
- A durable paste record with a unique identifier.
- Correct missing and expiration behavior.
- Basic size validation, reliability, and abuse controls.
...
- Recently created pastes should not be lost.
- Public reads should be responsive under a read-heavy workload.
- Expired content should not remain retrievable.
- Identifiers and abuse controls should not expose predictable private data.

## Scale assumptions

Provide these only when asked:

- Assume 10 million new pastes per month and 100 million reads per month.
- Paste text is limited to 1 MB and commonly below 10 KB.
- Expiration ranges from one hour to never.
...
- What size and content limits apply?
- Are pastes public, unlisted, or authenticated?
- Which expiration choices are supported?
- May a paste be edited after creation?
...
- What happens if two generated identifiers collide?
- Could expired content remain in a cache?
- How are abusive create requests limited?
- What does a reader see during storage failure?

## Examples of the allowed guidance boundary

- That choice creates the risk behind this question: What happens if two generated identifiers collide? How would you evaluate and mitigate it?
- Your current design has not resolved this failure mode: Could expired content remain in a cache? What change would you consider, and what tradeoff would it introduce?
...
Plausible families include relational rows keyed by random IDs, key-value records with TTLs, object storage for larger bodies, and cache-aside reads over a durable source of truth.
