...
export function App() {
  ...
  if (page.name === "summary") {
    return <SummaryPage summary={page.summary} onRestart={() => setPage({ name: "landing" })} />;
  }
  ...
  function begin(scenario: Scenario) {
    primeFeedbackAudio();
  ...
  return <ScenarioLanding onPrepare={preloadLiveSession} onStart={begin} />;
}

function SummaryPage({ summary, onRestart }: { summary: SessionSummary; onRestart: () => void }) {
  ...
  return (
    ...
    >
      <header className="flex flex-col items-start justify-between gap-4 border-b border-foreground pb-5 sm:flex-row print:pb-3">
        <div>
          <p className={`${reportLabelClass} mb-2 print:mb-1`}>Practice Complete</p>
          <h1 className="mb-2 max-w-3xl font-serif text-4xl leading-none tracking-tight sm:text-5xl print:text-[24pt]">
            {summary.scenarioTitle}
          </h1>
        </div>
        <div className="flex w-full gap-3 sm:w-auto print:hidden">
          <button className={`${buttonClass} flex-1 bg-card sm:flex-none`} onClick={() => window.print()}>Print</button>
          <button className={`${buttonClass} flex-1 bg-primary text-primary-foreground sm:flex-none`} onClick={onRestart}>Start over</button>
        </div>
      </header>

      <section className="mt-6 grid max-w-3xl gap-3 print:mt-3 print:max-w-none print:gap-2" aria-labelledby="practice-heading">
        <h2 id="practice-heading" className="font-serif text-2xl leading-none print:text-[14pt]">
      ...
      {conversationNotes.length ? (
        <section className="mt-6 grid max-w-3xl gap-3 print:mt-3 print:max-w-none print:gap-2" aria-labelledby="notes-heading">
          <h2 id="notes-heading" className="font-serif text-2xl leading-none print:text-[14pt]">Conversation notes</h2>
